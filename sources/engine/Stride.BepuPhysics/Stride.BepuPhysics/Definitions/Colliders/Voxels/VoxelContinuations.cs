...
/// <summary>
/// Collision batcher continuation for a convex shape tested against the children of a voxel shape.
/// </summary>
/// <remarks>
/// Children are computed on the fly: each child's convex data is written into the batcher's shape
/// cache when requested and discarded on flush. Manifolds are combined with a
/// <see cref="NonconvexReduction"/>, not <see cref="MeshReduction"/>, which is bound to the concrete
/// <see cref="Mesh"/> type; see the remarks on <see cref="VoxelCollider"/>.
/// </remarks>
    ...
    where TShape : unmanaged, IVoxelShape
{
    ...
    /// <summary>Materializes one child of the voxel shape into the batcher's shape cache. Also used by the compound handler.</summary>
...
/// <summary>The same, for a compound on the A side rather than a single convex.</summary>
